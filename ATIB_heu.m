function rel = ATIB_heu(bay , maxKnown , Tstar)

%     average_dep_time_cols=[c1    c2  ... first row is column number 
%                            ave1 ave2 ...   second row is ave_dep_time
%                            H1   H2   ...]  third row is height
...
while n <= N
    ...
    if ~retrieved(n)
        ...
        dest_col  = compute_ATIB(bay,maxKnown,blocking,col(n),Tstar,T);
        dest_col = dest_col(1);
        bay(bay==blocking)=0;
