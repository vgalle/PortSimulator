function [reloc,Moves]=myopic_and_LH(D,maxKnown,Tstar)
...
% First we compute its position in the bay, I and J are its coordinates
    ...
    % to move the top of column J denoted by r. We note this column C.
                ...
                if T<Tstar && r>K
                    candidate_cols=[];
                    candidate_cols(1,:) = 1:size(D,2);
                    candidate_cols(2,:) = H;
                    candidate_cols(:,candidate_cols(2,:)==size(D,1))=[];
                    candidate_cols(:,candidate_cols(1,:)==J ) =[];
                    [minHeight loc_minHeight] = min(candidate_cols(2,:));
                    C = candidate_cols(1,loc_minHeight);
